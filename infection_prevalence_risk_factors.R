...
serosurvey_surv_bl <- serosurvey_surv_bl %>% 
  mutate(SesScoreQnt = fct_relevel(SesScoreQnt, "3. med.", after = 0)) 
serosurvey_surv_bl <- serosurvey_surv_bl %>% 
...
SES <- serosurvey_surv_bl %>%
  ...
  summarise(
    total = n(),
    pcttotal = round((n() / sum(!is.na(serosurvey_surv_bl$SesScoreQnt)))*100,1),
    covid19 = sum(event),
    pctcases = round((sum(event) / sum(serosurvey_surv_bl$event[!is.na(serosurvey_surv_bl$SesScoreQnt)]))*100,1)
  )
education <- serosurvey_surv_bl %>%
  ...
  summarise(
    total = n(),
    pcttotal = round((n() / sum(!is.na(serosurvey_surv_bl$education))) * 100,1),
    covid19 = sum(event),
    pctcases = round((sum(event) / sum(serosurvey_surv_bl$event[!is.na(serosurvey_surv_bl$education)])) * 100,1)
  )
health_worker <- serosurvey_surv_bl %>%
  ...
  summarise(
    total = n(),
    pcttotal = round((n() / sum(!is.na(serosurvey_surv_bl$health_worker))) * 100,1),
    covid19 = sum(event),
    pctcases = round((sum(event) / sum(serosurvey_surv_bl$event[!is.na(serosurvey_surv_bl$health_worker)])) * 100,1)
  )
...
cox_model <- coxph(Surv(time, event) ~ agegr10 + sex, data = serosurvey_surv_bl)
...
# Fit the Cox regression model
cox_model <- coxph(Surv(time, event) ~ age + sex + SesScoreQnt, data = serosurvey_surv_bl)
# Extract hazard ratios and confidence intervals
hazard_ratios <- exp(coef(cox_model))
conf_intervals <- exp(confint(cox_model))
rounded_hazard_ratios <- round(hazard_ratios, 2)
